...
for i in buttons:
    if c < 2:
        tk.Button(root, text = i, command = lambda i=i: sum_value.set(i), padx = 5, pady = 3).grid(row = r, column = c, sticky = (N,S,E,W))        
        c += 1
    else:
        tk.Button(root, text = i, command = lambda i=i: sum_value.set(i), pady = 3).grid(row = r,column = c,sticky = (N,S,E,W))
        r  += 1
        c = 0
